...
def generate_pdf_report(batch_id: int, results: dict) -> list[dict]:
    """第一次审核只生成考勤明细PDF（自动识别保安/保洁）。"""
    report_dir = Path(settings.report_dir) / str(batch_id)
    report_dir.mkdir(parents=True, exist_ok=True)
    reports = []
    reports.append(build_attendance_base_pdf(batch_id, report_dir, results))
    return reports
...
def is_serious_exception(row: dict) -> bool:
    text = f"{row.get('status', '')}{row.get('exception_type', '')}{row.get('exception_reason', '')}"
...
def build_ai_pdf(batch_id: int, report_dir: Path, results: dict) -> dict:
    ...
    story.extend(simple_table([
        ...
        ["确认后异常率", f"{confirmed_exception_rate}%", "确认异常条数", summary.get("confirmed_count", 0), "未确认条数", summary.get("skipped_count", 0)],
        ["总扣款金额", f"{total_deduction}元", "考勤扣款", f"{summary.get('attendance_deduction_amount', 0)}元", "岗位扣款", f"{summary.get('position_deduction_amount', 0)}元"],
    ], [24*mm, 32*mm, 24*mm, 32*mm, 24*mm, 32*mm]))
    story.append(Paragraph("三、审核总结（直接复制）", s["CNHeading"]))
    story.append(Paragraph(build_copyable_summary(results), s["CNBody"]))
    story.append(Paragraph("四、排班审核及扣款明细", s["CNHeading"]))
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph("本报告由 AI 考勤审核系统自动生成，请结合合同、排班表、BI原始考勤和现场管理记录复核确认。", s["CNNote"]))
    doc.build(story)
    return {"report_type": title, "file_format": "pdf", "file_path": str(file_path), "download_url": f"/files/reports/{batch_id}/{title}.pdf"}
...
def build_deduction_pdf(batch_id: int, report_dir: Path, results: dict) -> dict:
    ...
    story.append(Paragraph("二、异常率对比", s["CNHeading"]))
    story.extend(simple_table([
        ["排班任务", schedule_count, "第一次异常数", first_exception_count, "第一次异常率", f"{first_exception_rate}%"],
        ["确认异常条数", confirmed_count, "未确认条数", skipped_count, "确认后异常率", f"{confirmed_exception_rate}%"],
        ["总扣款金额", f"{total_deduction}元", "考勤扣款", f"{summary.get('attendance_deduction_amount', 0)}元", "岗位扣款", f"{summary.get('position_deduction_amount', 0)}元"],
    ], [24*mm, 32*mm, 24*mm, 32*mm, 24*mm, 32*mm]))
    ...
    if deduction_summary:
        story.append(Paragraph("五、人员扣款汇总", s["CNHeading"]))
        story.extend(table_flow(deduction_summary[:30], "deduction_summary"))

    story.append(Spacer(1, 8))
    story.append(Paragraph("本报告由 AI 考勤审核系统自动生成，扣款金额按合同细则计算，请结合项目确认结果复核。", s["CNNote"]))
    doc.build(story)
    return {"report_type": title, "file_format": "pdf", "file_path": str(file_path), "download_url": f"/files/reports/{batch_id}/{title}.pdf"}
